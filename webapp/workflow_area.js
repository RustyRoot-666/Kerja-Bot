...
async function searchGlobalOpenInet(action, rawQuery) {
  ...
  if (!user?.id) { result.innerHTML = '<div class="empty"><p>❌ Mini App harus dibuka dari Telegram.</p></div>'; return; }
  if (q.length < 6) { result.innerHTML = '<div class="empty"><p>Masukkan minimal 6 digit nomor INET.</p></div>'; input?.focus(); return; }
  ...
  try {
    ...
    if (!orders.length) { result.innerHTML = '<div class="empty"><p>INET OPEN tidak ditemukan.</p></div>'; return; }
    orders.forEach(order => {
      const button = document.createElement('button'); button.className = 'tool-action';
      ...
      button.addEventListener('click', () => { const selected = { ...order, area: order.area || 'LAINNYA' }; state.workflow = { action, order: selected }; renderWorkflowForm(action, selected); });
      result.appendChild(button);
    });
  } catch (error) { result.innerHTML = `<div class="empty"><p>❌ ${esc(error.message)}</p></div>`; }
}
...
function renderWorkflowAreas(action, payload) {
  ...
  host.innerHTML = `<article class="tool-card">
    ...
    <div class="tool-card" style="margin-top:12px;border-color:#27547a;background:linear-gradient(180deg,#0b2238,#091927)">
      ...
      <div class="search-wrap" style="margin-top:10px"><span>⌕</span><input id="wfGlobalInet" inputmode="numeric" autocomplete="off" placeholder="Masukkan nomor INET..." /></div>
  ...
  searchInput?.addEventListener('keydown', event => { if (event.key === 'Enter') { event.preventDefault(); searchGlobalOpenInet(action, event.currentTarget.value); } });
  const list = document.querySelector('#wfAreaList');
  if (!areas.length) list.innerHTML = '<div class="empty"><p>✅ Tidak ada order OPEN milik teknisi ini. Pencarian INET global tetap dapat digunakan.</p></div>';
    ...
    button.addEventListener('click', () => renderWorkflowAreaOrders(action, area)); list.appendChild(button);
  });
  document.querySelector('#wfBackHome')?.addEventListener('click', renderWorkflowHome);
}

function renderWorkflowAreaOrders(action, area) {
  const rows = (area.orders || []).map(order => ({ ...order, area: area.area })); renderWorkflowOrders(action, rows);
  const card = workflowHost().querySelector('.tool-card'); if (!card) return;
  const heading = card.querySelector('strong'); if (heading) heading.textContent = `${action.toUpperCase()} • ${area.area}`;
  const small = card.querySelector('small'); if (small) small.textContent = `${rows.length} order OPEN • pilih INET yang akan dikerjakan`;
  const back = card.querySelector('#wfBack');
  if (back) { back.innerHTML = `<b>‹ Kembali ke area</b><span>📍 ${esc(area.area)}</span>`; back.replaceWith(back.cloneNode(true)); card.querySelector('#wfBack')?.addEventListener('click', () => renderWorkflowAreas(action, state.myOpenOrders)); }
}
...
startWorkflow = async function startWorkflowAreaFirst(action) {
  ...
  try { const payload = state.myOpenOrders || await fetchMyOpenOrders(false); renderWorkflowAreas(action, payload); }
  catch (error) { host.innerHTML = `<div class="empty"><p>❌ ${esc(error.message)}</p><button class="tool-action" id="wfBack"><b>Kembali</b><span>‹</span></button></div>`; document.querySelector('#wfBack')?.addEventListener('click', renderWorkflowHome); }
};
...
renderWorkflowForm = function renderWorkflowFormWithAreaContext(action, order) {
  ...
  const article = workflowHost()?.querySelector('.tool-card'); if (!article) return;
  const ticketBlock = article.querySelector('.info-box, .tool-card'); const context = document.createElement('div'); context.className = 'info-box'; context.style.marginTop = '12px';
...
// Load the address-category override after all app functions are defined.
(function(){ const s=document.createElement('script'); s.src='/website/order_area_filter.js?v=20260906-address4'; s.async=false; document.head.appendChild(s); })();
