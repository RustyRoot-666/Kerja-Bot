...
function web_report_status(array $o): string { return norm($o['result']??$o['status']??$o['hasil']??''); }
...
function web_replacement_norm_inet(string $value): string { return preg_replace('/\D+/', '', trim($value)); }
function web_replacement_header_map(array $headers): array { $map=[]; foreach($headers as $i=>$h)$map[norm($h)]=$i; return$map; }
function web_replacement_col(array $map,array $aliases): ?int { foreach($aliases as $alias){$k=norm($alias);if(array_key_exists($k,$map))return$map[$k];}return null; }

function web_replacement_status_data(): array {
    $report=[];
    if(table_exists('report_group_orders'))try{foreach(db()->query('SELECT service_number FROM report_group_orders')->fetchAll() as $r){$n=web_replacement_norm_inet((string)($r['service_number']??''));if($n!=='')$report[$n]=true;}}catch(Throwable $e){error_log('[miniapp-php] replacement report status unavailable: '.$e->getMessage());}
    $updates=[];
    if(table_exists('kendala_updates'))try{
        foreach(db()->query('SELECT service_number,status,rca,id FROM kendala_updates ORDER BY id ASC')->fetchAll() as $r){
            $n=web_replacement_norm_inet((string)($r['service_number']??''));if($n==='')continue;
            $updates[$n][]=['status'=>norm($r['status']??''),'rca'=>norm($r['rca']??''),'id'=>(int)($r['id']??0)];
        }
...
function web_replacement_rows(): array {
    ...
    $inetCol=web_replacement_col($map,['INET','NO INET','NO INTERNET','NO SERVICE']);$timCol=web_replacement_col($map,['TIM']);
    $nameCol=web_replacement_col($map,['NAMA PETUGAS','PETUGAS','TEKNISI','NAMA TEKNISI']);$dateCol=web_replacement_col($map,['TANGGAL','DATE','TGL']);$ticketCol=web_replacement_col($map,['TICKET','TIKET']);
    $addressCol=web_replacement_col($map,['ALAMAT']);$customerCol=web_replacement_col($map,['NAMA']);$phoneCol=web_replacement_col($map,['CP','NO HP','NOMOR HP']);$rcaSheetCol=web_replacement_col($map,['RCA']);
    [$report,$updates]=web_replacement_status_data();$out=[];
    foreach(array_slice($rows,$headerIndex+1) as $row){
        $inet=web_replacement_norm_inet((string)($row[$inetCol]??''));$tim=norm($row[$timCol]??'');if($inet===''||$tim!=='REPLACEMENT')continue;
...
function web_replacement_technician_matches(array $order,array $tech): bool {
    $sheetName=norm_name((string)($order['technician_name']??''));if($sheetName==='')return false;$techName=norm_name((string)($tech['name']??''));
    if($sheetName!==''&&$sheetName===$techName)return true;
    try{if(function_exists('technician_master_resolve')){$master=technician_master_resolve('',(string)($order['technician_name']??''));if($master&&norm_name((string)($master['canonical_name']??''))===$techName)return true;}}catch(Throwable $e){}
    return false;
}
function web_replacement_orders_for_tech(array $tech,string $area,string $period): array {
    if(!web_report_area_match((string)$tech['sto'],$area))return[];$orders=[];
    foreach(web_replacement_rows() as $o)if(web_replacement_technician_matches($o,$tech)&&web_report_day_match((string)($o['raw_day']??''),$period)){$o['technician_nik']=$tech['nik'];$o['technician_name']=$tech['name'];$o['sto']=$tech['sto'];$orders[]=$o;}
    return$orders;
}
...
function web_report_selected(int $viewerTelegramId,string $nik,string $area,string $period): array {
    ...
    usort($orders,fn($a,$b)=>($c=strcmp((string)$b['date'],(string)$a['date']))!==0?$c:strcmp((string)$a['service_number'],(string)$b['service_number']));
...
function web_report_sheet_rows(): array {
    $raw=@file_get_contents(sheet_csv_url());if($raw===false||trim($raw)==='')throw new RuntimeException('Google Sheets tidak dapat dibaca.');$fp=fopen('php://temp','r+');fwrite($fp,preg_replace('/^\xEF\xBB\xBF/','',$raw));rewind($fp);$rows=[];while(($r=fgetcsv($fp))!==false)$rows[]=$r;fclose($fp);$norms=fn($v)=>norm($v);$serviceAliases=['NO INET','NO INTERNET','NO SERVICE','SERVICE NUMBER','INTERNET NUMBER','INET'];$headerIndex=-1;$serviceCol=null;foreach(array_slice($rows,0,30,true) as $i=>$r){foreach($r as $j=>$h)if(in_array($norms($h),$serviceAliases,true)){$serviceCol=$j;break;}if($serviceCol!==null){$headerIndex=$i;break;}}if($headerIndex<0)throw new RuntimeException('Kolom INET tidak ditemukan di Google Sheet.');return['headers'=>$rows[$headerIndex],'rows'=>array_slice($rows,$headerIndex+1),'service_col'=>$serviceCol];
}
function web_report_export_rows(int $viewerTelegramId,string $nik,string $area,string $period): array {$selected=web_report_selected($viewerTelegramId,$nik,$area,$period);if(!($selected['ok']??false))return$selected;$keys=[];foreach($selected['orders'] as $o){$k=norm_key($o['service_number']??'');if($k!=='')$keys[$k]=true;}$sheet=web_report_sheet_rows();$out=[];foreach($sheet['rows'] as $row){$service=(string)($row[$sheet['service_col']]??'');$k=norm_key($service);if($k!==''&&isset($keys[$k]))$out[]=$row;}return['ok'=>true,'headers'=>$sheet['headers'],'rows'=>$out,'technician'=>$selected['technician'],'period'=>$period,'area'=>strtoupper($area),'matched'=>count($out),'reported'=>count($selected['orders'])];}
function web_report_xml(string $v): string{return htmlspecialchars($v,ENT_QUOTES|ENT_XML1,'UTF-8');}
function web_report_excel(array $headers,array $rows): string {$xml='<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?>';$xml.='<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Laporan"><Table>';$all=array_merge([$headers],$rows);foreach($all as $r){$xml.='<Row>';foreach($r as $v)$xml.='<Cell><Data ss:Type="String">'.web_report_xml((string)$v).'</Data></Cell>';$xml.='</Row>';}return$xml.'</Table></Worksheet></Workbook>';}
