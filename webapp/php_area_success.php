...
function area_success_locality(string $address): string {
    ...
    // Apartments are grouped by physical complex. Tower, unit, floor, block,
    // house/unit numbers and trailing administrative locality are not areas.
    if (preg_match('/^(?:APARTEMEN|APARTEMENT|APARTMENT)\s+(.+)$/i', $s, $m)) {
        $building = trim($m[1]);
        $building = preg_replace('/\s+(?:TOWER|UNIT|LT|LANTAI)\b.*$/i', '', $building) ?: $building;
        $building = preg_replace('/\s+[A-Z]\s+(?:UTARA|SELATAN|TIMUR|BARAT)\b.*$/i', '', $building) ?: $building;

        $upper = strtoupper(trim($building));
        $known = [
            'BALE HINGGIL' => 'APARTEMEN BALE HINGGIL',
            'DIAN REGENCY' => 'APARTEMEN DIAN REGENCY',
            'EDUCITY' => 'APARTEMEN EDUCITY',
            'ONE GALAXY' => 'APARTEMEN ONE GALAXY',
            'PUNCAK KERTAJAYA' => 'APARTEMEN PUNCAK KERTAJAYA'
        ];
        foreach ($known as $needle => $canonical) {
            if ($upper === $needle || str_starts_with($upper, $needle . ' ')) return $canonical;
        }

        // Common tower names are part of the apartment complex, not an area.
        $building = preg_replace('/\s+(?:HARVARD|YALE|PRINCETON|STAMFORD)\b.*$/i', '', $building) ?: $building;
        $building = preg_replace('/\s+[A-Z]$/i', '', $building) ?: $building;
        return strtoupper('APARTEMEN ' . trim($building));
    }
