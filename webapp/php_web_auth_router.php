...
function web_auth_same_origin(): void {
    $origin=trim((string)($_SERVER['HTTP_ORIGIN']??''));if($origin==='')return;$parsed=parse_url($origin);
    ...
    $originScheme=strtolower((string)$parsed['scheme']);$originHost=strtolower((string)$parsed['host']);$originPort=isset($parsed['port'])?(int)$parsed['port']:null;
    $forwardedProto=strtolower(trim(explode(',',(string)($_SERVER['HTTP_X_FORWARDED_PROTO']??''))[0]??''));$serverScheme=$forwardedProto!==''?$forwardedProto:((!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')?'https':'http');
    $serverHost=strtolower(trim(explode(',',(string)($_SERVER['HTTP_X_FORWARDED_HOST']??''))[0]??''));if($serverHost==='')$serverHost=strtolower((string)($_SERVER['HTTP_HOST']??''));$serverHost=preg_replace('/:\d+$/','',$serverHost)?:$serverHost;
    $allowedHosts=['app.botkerja.web.id'];$currentHost=preg_replace('/:\d+$/','',strtolower((string)($_SERVER['HTTP_HOST']??'')));if($currentHost!=='')$allowedHosts[]=$currentHost;
